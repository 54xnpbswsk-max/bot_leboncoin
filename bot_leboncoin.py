...
CHAT_IDS = os.environ["CHAT_IDS"].split(",")  # plusieurs destinataires séparés par une virgule
...
# --- URL Leboncoin à scraper (avec tes filtres) ---
...
while True:
    ...
    for ad in ads:
        ...
        if link not in seen:
            # Envoie Telegram avec titre et lien
            send(f"🚗 {title}\n{link}")
            seen.add(link)
    ...
    time.sleep(30)  # intervalle de vérification en secondes
